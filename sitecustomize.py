...
def _runtime_guard_enabled() -> bool:
    """Feature flag to enable/disable import-hook patching.

    Default: enabled.
    Disable by setting `RHK_ENABLE_RUNTIME_GUARD=0` (or false/off/no).
    """
    raw = str(os.getenv("RHK_ENABLE_RUNTIME_GUARD", "1") or "").strip().lower()
...
class _GuardedLoader(importlib.abc.Loader):
    ...
    def create_module(self, spec: importlib.machinery.ModuleSpec) -> Optional[ModuleType]:
        ...
        create_module_fn = getattr(self.orig, "create_module", None)
        if callable(create_module_fn):
            module = create_module_fn(spec)
            if module is None or isinstance(module, ModuleType):
                return module
        return None

    def exec_module(self, module: ModuleType) -> None:
        # 1. Load the module normally
        exec_module_fn = getattr(self.orig, "exec_module", None)
        if callable(exec_module_fn):
            exec_module_fn(module)
        else:
...
# Run installation
if __name__ != "__main__" and _runtime_guard_enabled():
    _install_guard()
