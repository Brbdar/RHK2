...
from rhk_export_paths import ensure_export_dir
...
def _arrow(flag: Optional[str]) -> str:
    ...
    # ASCII only (copy/paste safe with core PDF fonts).
...
def _draw_bullets_wrapped(
    ...
    *,
    ...
    for b in bullets_in:
        ...
        # ASCII bullet for reliable PDF copy/paste with core fonts
...
def _draw_prev_rhk_base_compact(
    ...
    *,
    ...
    if _fmt_num(prev_ci) is not None:
        right.append(("CI", f"{_fmt_num(prev_ci)} L/min/m2"))
    ...
    ci_s = _fmt(prev_ci, unit="L/min/m2", ndigits=1) or "-"
...
def generate_prerhk_pdf(case_state: Dict[str, Any]) -> str:
    ...
    allergies_other_text = str(ui.get("allergies_other_text") or "").strip()
    ...
    prev_rhk_date = str(ui.get("prev_rhk_date") or "").strip()
    ...
    prev_label = str(ui.get("prev_label") or "").strip()
    ...
    _pasp = _echo_num(ui.get("pasp_echo"))  # noqa: F841 – reserved
    ...
    if ra_esa is not None and str(ra_esa).strip() != "":
        echo_pairs.append(_echo_line("RA ESA", ra_esa, "cm2", 0))
    ...
    # --- Create PDF ---
    # Use the central export resolver so runtime profiles and cleanup stay aligned.
    out_dir = str(ensure_export_dir())
    ...
    if need_bottom_strip:
        if need_handwrite_ox:
            ...
            min_strip = 38 * mm
            _pref_strip = 50 * mm  # noqa: F841 – reserved for layout tuning
            candidates = [70, 68, 66, 64, 62, 60, 58, 56, 54, 52, 50, 48, 46, 44, 42, 40, 38]
        else:
            min_strip = 20 * mm
            _pref_strip = 24 * mm  # noqa: F841 – reserved for layout tuning
