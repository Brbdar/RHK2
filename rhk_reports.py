...
def _no_congestion_context(ui: Dict[str, Any], der: Dict[str, Any]) -> bool:
    ...
    The function gates downstream filtering of contradictory phrases like
    "Bei führender Stauung". It must NOT fire when congestion data is simply
    missing (no RAP / no IVC / no PAWP) — in that case the absence of a
    positive flag does not mean absence of congestion, and we keep the
    template content as-is rather than silently editing it out.
    """
    der_d = der or {}
    ui_d = ui or {}
    ...
    # Both axes must be (a) assessable and (b) negative.
...
def _filter_narrative_block(text: str, ui: Dict[str, Any], der: Dict[str, Any]) -> str:
    ...
    # Only strip the *bare* "Aktuell RHK in Ruhe." sentence — the extended
    # variants ("Aktuell RHK in Ruhe mit ergänzender Belastungsmessung.",
    # "Aktuell RHK in Ruhe mit Vasoreaktivitätstestung.", …) carry information
    # the referring physician needs and must remain intact. An earlier version
    # stripped the "Aktuell RHK in Ruhe" prefix unconditionally, producing
    # orphaned fragments like ". mit ergänzender Belastungsmessung."
...
def _format_warning_item(w: Any) -> str:
    ...
    if isinstance(w, dict):
        ...
        try:
            ...
            log_exception("RHK_REP_WARN_ITEM_FORMAT", "Warning item value formatting failed.", exc)
        return msg
    return str(w).strip()
