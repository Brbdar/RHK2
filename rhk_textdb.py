...
Enthält:
- "Katalog v1.x (K01–K21)" als Paket-Bausteine (jeweils Beurteilung + Empfehlung)
...
class TextBlock:
    ...
    P-Module metadata (optional, defaults keep backward compatibility):
      level_default  – Empfohlene Priorität, wenn das Modul ohne weitere
                       Heuristik angeboten werden soll: 1 = prioritär,
                       2 = sinnvoll ergänzend, 3 = optional. `compute_p_module_policy`
                       darf die Priorität pro Fall hochstufen, aber
                       niemals unter das Default senken, ohne dass
                       eine Disable-Regel zieht.
      clinical_group – Frei wählbarer Kategorie-Tag ("diagnostic",
                       "therapy", "monitoring", "workup", "lifestyle",
                       "specialist_referral", "misc"). Nur zur Gruppierung
                       in UI und zur Doc-Generierung verwendet; beeinflusst
                       NICHT die Selection-Logik.
      required_context – Liste von Placeholder-Namen, auf die das Template
                       zwingend angewiesen ist. Wird vom Renderer ausgewertet
                       um stub-gerenderte Module zu unterdrücken, wenn der
                       benötigte Kontext fehlt (z.B. `therapy_plan_sentence`
                       für P03-P06). Leere Liste = keine Abhängigkeit.
    """
    id: str
    title: str
    applies_to: str
...
B_BLOCKS: Dict[str, TextBlock] = {
    "B01": TextBlock(
        ...
        template=(
            "Normale pulmonale Druck- und Widerstandswerte {ci_phrase}{co_method_phrase}. "
    ...
    "B03": TextBlock(
        ...
        template=(
            "Keine pulmonale Druck- und Widerstandserhöhung in Ruhe {ci_phrase}{co_method_phrase}. "
    ...
    "B04": TextBlock(
        ...
        template=(
            "Leichtgradig erhöhte pulmonale Druckwerte bei {pvr_phrase} und {ci_phrase}{co_method_phrase}. "
            "{cv_stauung_phrase} {pv_stauung_phrase} {step_up_sentence} "
    ...
    "B05": TextBlock(
        ...
        template=(
            ...
            "({mpap_phrase}, {pawp_phrase}, {pvr_phrase}) {ci_phrase}{co_method_phrase}. "
            "{cv_stauung_phrase} {pv_stauung_phrase} {step_up_sentence}"
        ),
        category="B",
    ),
    ...
    "B09": TextBlock(
        ...
        template=(
            ...
            "{ci_phrase}{co_method_phrase}. {cv_stauung_phrase} {step_up_sentence} "
...
P_BLOCKS: Dict[str, TextBlock] = {
    "P01": TextBlock(        id="P01",        title="PH Basisdiagnostik komplettieren",        applies_to="Erstdiagnose, Reevaluation oder unklare Ätiologie",        template=(
            "PH-Basisdiagnostik komplettieren (falls noch nicht erfolgt):\n"
            "• Echo (RV-Funktion, TR-Grad, Klappen)\n"
            "• Lufu inkl. DLCO, BGA\n"
            "• V/Q-Szintigraphie (CTEPD/CTEPH) + CT Thorax nach Fragestellung\n"
            "• Basislabor inkl. BNP/NT-proBNP; Autoimmun-/Infektionsdiagnostik nach Kontext\n"
    ...
    "P02": TextBlock(
        id="P02",
        title="Diurese (aktive Dekongestion)",
        applies_to="ZVD, Ödeme, Gewichtszunahme oder kardiorenale Dynamik",
        template=(
            "Bei klinischer Stauung Diuretikatherapie mit Ziel Euvolämie anpassen.\n"
            "Kontrolle: Gewicht, Kreatinin, Elektrolyte; bei Bedarf stationär i.v. Diurese."
        ),
        category="P",
        level_default=1,
        clinical_group="therapy",
        notes="Aktive Dekongestion bei vorhandener Stauung. Für stabile Euvolämie-Maintenance siehe P40.",
    ),
    "P03": TextBlock(
        id="P03",
        title="PAH-Therapie: PDE5-Inhibitor initiieren",
        applies_to="Präkapilläre PH Gruppe 1 ohne Gruppe-2-Dominanz und ohne Riociguat",
        template=(
            "PDE5-Inhibitor als Baustein der Initialtherapie bei PAH Gruppe 1 "
            "(Sildenafil oder Tadalafil) gemäß Fachinformation und hausinterner SOP.\n"
            "{therapy_plan_sentence}\n"
            "Monitoring: Blutdruck (Hypotonie-Risiko), Verträglichkeit (Flush, Kopfschmerz, Dyspepsie), "
            "Sehstörungen. Wechselwirkungen mit Nitraten und alpha-Blockern beachten. "
            "Kontraindikation: gleichzeitige Riociguat-Therapie (→ P05)."
        ),
        category="P",
        level_default=1,
        clinical_group="therapy",
        required_context=("therapy_plan_sentence",),
        notes="Medikamenten- und Dosisdetails aus planned_actions; Template liefert immer klinischen Kontext.",
        variants={
            "example_pde5i": (
                "Beginn PDE5-Inhibitor gemäß Fachinformation und hausinterner SOP. "
                "Kontrolle Blutdruck und Verträglichkeit zeitnah."
            ),
        },
    ),
    "P04": TextBlock(
        id="P04",
        title="PAH-Therapie: ERA initiieren oder erweitern",
        applies_to="PAH Gruppe 1, Initialtherapie oder Erweiterung zur dualen oralen Kombination",
        template=(
            "Endothelin-Rezeptor-Antagonist (Ambrisentan oder Macitentan) als Komponente der "
            "oralen Kombinationstherapie bei PAH gemäß Fachinformation und SOP.\n"
            "{therapy_plan_sentence}\n"
            "Monitoring: Transaminasen (initial 4-wöchentlich, dann quartalsweise), Hb, Beinödeme; "
            "Schwangerschaft ausschließen (teratogen → P21). Bei gleichzeitiger Leberwert-Erhöhung "
            "siehe P41."
        ),
        category="P",
        level_default=1,
        ...
        variants={
            "example_era": (
                "Initiierung oder Ergänzung ERA gemäß Fachinformation und hausinterner SOP. "
                "Kontrollen je nach Präparat."
            ),
        },
    ),
    "P05": TextBlock(
        id="P05",
        title="Riociguat (CTEPH / inoperabel)",
        applies_to="CTEPH inoperabel/persistierend oder dokumentierte PDE5i-Intoleranz",
        template=(
            "Riociguat als sGC-Stimulator bei CTEPH inoperabel bzw. persistierend nach PEA/BPA "
            "oder als Alternative zu PDE5-Inhibitoren nach lokaler SOP.\n"
            "{therapy_plan_sentence}\n"
            "Dosierung einschleichend nach Blutdrucktoleranz bis Zielhöchstdosis. "
            "Kontraindiziert in Kombination mit PDE5-Inhibitoren und Nitraten; "
            "Washout-Zeiten (24 h Sildenafil, 48 h Tadalafil) konsequent einhalten. "
            "Monitoring: Blutdruck, Synkopen, Hämoptysen."
        ),
        category="P",
        level_default=1,
        clinical_group="therapy",
        ...
        variants={
            "example_riociguat": (
                "Riociguat einschleichend gemäß Fachinformation und hausinterner SOP. "
                "Kombination mit PDE5-Inhibitor kontraindiziert; Washout-Zeiten beachten."
            ),
        },
    ),
    "P06": TextBlock(
        id="P06",
        title="Therapie-Eskalation bis parenteral prüfen",
        applies_to="Intermediär-hohes oder hohes Risiko (ESC/ERS), RV-Versagen oder Progress trotz dualer oraler Therapie",
        template=(
            "Prüfung einer Therapie-Eskalation bei persistierend hohem Risiko oder Progression "
            "trotz oraler Kombination. Optionen je nach Profil: Triple-Therapie (Hinzunahme "
            "parenteraler Prostacyclin-Analoga wie Treprostinil s.c./i.v. oder Epoprostenol i.v.), "
            "Selexipag oral als Prostacyclin-Rezeptor-Agonist.\n"
            "{therapy_escalation_sentence}\n"
            "Entscheidung ausschließlich im PH-Zentrum / PH-Board unter Einbezug von "
            "Risikostratifizierung (ESC/ERS 4-Strata, REVEAL), RV-Funktion und Patientenpräferenz. "
            "Bei refraktärem RV-Versagen: frühzeitige Transplantations-Evaluation (→ P25)."
        ),
        category="P",
        level_default=1,
        clinical_group="specialist_referral",
        required_context=("therapy_escalation_sentence",),
        variants={
            "generic": (
                "Eskalationsoptionen im PH-Zentrum besprechen inkl. parenteraler Optionen. "
                "Entscheidung im interdisziplinären Setting."
            ),
        },
    ),
    "P07": TextBlock(        id="P07",        title="Studien Option prüfen",        applies_to="wenn lokal verfügbar",        template=(
            "Prüfung einer Studieneinschlussoption im PH-Zentrum gemäß aktueller Verfügbarkeit und Kriterien."
        ),
        category="P",
    ),
    ...
    "P10": TextBlock(        id="P10",        title="Antikoagulation und Gerinnungsambulanz",        applies_to="VTE, Perfusionsdefekt, APS Verdacht oder Rezidiv",        template=(
            ...
            "Gerinnungsambulanz zur Einordnung und Optimierung der AK."
        ),
        category="P",
        variants={
            "generic": (
    ...
    "P11": TextBlock(        id="P11",        title="Verlaufskontrolle (Therapieänderung / instabile Lage)",        applies_to="Erstdiagnose, Therapieänderung oder instabile Lage — nicht als Ersatz für strukturierten PH-Ambulanz-Turnus (siehe P49/P51)",        template=(
            "Kontrolle im PH-Zentrum in {followup_timing_desc} mit Reevaluation des Risikos und Echo-Verlauf.\n"
    ...
    "P13": TextBlock(        id="P13",        title="Anämie / Eisenmangel: diagnostische Abklärung",        applies_to="Anämie oder Eisenmangelverdacht — für Substitution siehe P42",        template=(
    ...
    "P18": TextBlock(        id="P18",        title="Infektiologisches Screening",        applies_to="assoziierte PAH DD oder unklare präkapilläre PH",        template=(
            "Infektiologisches Screening (z.B. HIV, Hepatitis) gemäß lokaler SOP und Anamnese durchführen und dokumentieren."
        ),
        category="P",
    ),
    "P19": TextBlock(
    ...
    "P25": TextBlock(        id="P25",        title="Eskalationszentrum und Transplantation prüfen",        applies_to="Hochrisiko oder Progress trotz Therapie",        template=(
            "Bei Hochrisiko oder Progress: Vorstellung im PH-Zentrum zur Eskalationsstrategie inkl. parenteraler Optionen und ggf. Transplantationsabklärung."
        ),
        category="P",
    ),
    ...
    "P29": TextBlock(        id="P29",        title="LTOT: Indikation und Verordnung",        applies_to="Hypoxämie mit Indikation zur Langzeitsauerstofftherapie — für Reevaluation bereits verordneter LTOT siehe P48",        template=(
    ...
    "P32": TextBlock(
        ...
        template=(
            "### Hyperzirkulation / flussdominante Druckerhöhung\n"
            "{pressure_resistance_short}{co_method_phrase}.\n"
            "Konstellation vereinbar mit flussdominanter Druckerhöhung bei hoher Vorwärtsleistung und niedriger PVR.\n"
            "Leber/Portal-Kontext: {liver_ph_profile_label}\n"
            "{step_up_sentence}\n"
            "Kontext-/Ursachenklärung: Anämie, Hyperthyreose, Shunt, Leber/Portal, AV-Fistel.\n"
            "Therapeutisch primär Auslöser und Volumenstatus adressieren; Reevaluation der Hämodynamik nach Optimierung.\n"
            "Hinweis: Bei isolierter Flusserhöhung (PVR ≤ 2 WU) ist PH-spezifische Vasodilatation in der Regel nicht indiziert; Entscheidung im PH-Zentrum."
        ),
        category="P",
    ),
    "P33": TextBlock(
    ...
    "P40": TextBlock(
        id="P40",
        title="Volumenmanagement (Euvolämie-Maintenance)",
        applies_to="Stabile Situation, Erhalt der Euvolämie im ambulanten Verlauf — für aktive Dekongestion siehe P02",
        template=(
            "Erhaltung der Euvolämie im ambulanten Verlauf. "
            "Tägliches Gewicht dokumentieren, Diuretika-Dosis nach Bedarf anpassen. "
            "Quartalsweise Verlauf von Kreatinin, Elektrolyten und NT-proBNP; "
            "bei Zeichen einer erneuten Dekompensation frühzeitige Diuretika-Intensivierung (→ P02)."
        ),
        category="P",
        level_default=2,
        clinical_group="monitoring",
        notes="Ambulante Maintenance, klar abgegrenzt gegen P02 (aktive Dekongestion).",
    ),
    "P41": TextBlock(
        id="P41",
    ...
    "P42": TextBlock(
        id="P42",
        title="Eisenmangel: Substitutionstherapie",
        applies_to="Bestätigter Eisenmangel (Ferritin < 100 µg/l oder TSAT < 20%) — diagnostische Abklärung siehe P13",
        template=(
            "Bei bestätigtem Eisenmangel Substitutionstherapie zur Verbesserung der "
            "Belastungsfähigkeit: in der Regel parenteral (Ferric Carboxymaltose oder "
            "Ferric Derisomaltose) gemäß Gewichts-adaptiertem Dosierschema; orale "
            "Substitution nur bei geringer Mangelausprägung und guter Verträglichkeit. "
            "Kontrolle Ferritin, TSAT und Hb nach 4–8 Wochen; Wiederholung bei erneutem Abfall."
        ),
        category="P",
        level_default=2,
        clinical_group="therapy",
        notes="Therapeutische Fortsetzung von P13 (Diagnostik).",
    ),
    "P43": TextBlock(
        id="P43",
    ...
    "P48": TextBlock(
        id="P48",
        title="LTOT: Reevaluation & Flow-Anpassung",
        applies_to="LTOT bereits verordnet, Optimierung von Flow und Tragezeit — Neu-Indikation siehe P29",
        template=(
            "Konsequente LTOT-Anwendung ≥ 16 h/Tag prüfen. "
            "Flow-Anpassung nach BGA in Ruhe, unter Belastung und in der Nachtmessung "
            "(Zielsättigung SpO₂ ≥ 90 %). Adhärenz und Geräte-Akzeptanz erfragen, "
            "ggf. Wechsel auf mobiles Konzentratorsystem. Reevaluation bei persistierender "
            "Dyspnoe trotz adäquater Oxygenierung — DD kardiale Limitation, Schlafapnoe (→ P46)."
        ),
        category="P",
        level_default=2,
        clinical_group="monitoring",
        notes="Optimierung bestehender LTOT; Neu-Indikation läuft über P29.",
    ),
    "P49": TextBlock(
        id="P49",
        title="PH-Ambulanz: Hochrisiko-Monitoring (3 Monate)",
        applies_to="PH mit intermediär-hohem oder hohem Risiko (ESC/ERS 4-Strata) — nicht parallel zu P51",
        template=(
    ...
    "P51": TextBlock(
        id="P51",
        title="PH-Ambulanz: Routineverlauf (6 Monate)",
        applies_to="PH mit niedrigem Risiko (ESC/ERS 4-Strata: low / intermediate-low) — nicht parallel zu P49",
        template=(
...
G_BLOCKS: Dict[str, TextBlock] = {
    "G01": TextBlock(
        ...
        template=(
            ...
            "Komorbiditäten und Patient*innenpräferenz erfolgen."
        ),
        category="G",
    ),
    "G02": TextBlock(
...
# ---------------------------------------------------------------------------
# NEU: "Katalog v1.x – K01..K21" als Pakettexte (Beurteilung + Empfehlung)
...
_add_k_package(
    ...
    beurteilung=(
        "Keine pulmonale Druck- und Widerstandserhöhung bei {ci_phrase}{co_method_phrase}. "
        "{cv_stauung_phrase} {pv_stauung_phrase} {step_up_sentence} "
...
_add_k_package(
    ...
    empfehlung=(
        "Keine PH in Ruhe. Belastungsreaktion spricht eher für linkskardiale Limitation als primär pulmonalvaskulär. "
...
_add_k_package(
    ...
    beurteilung=(
        "In Ruhe {rest_ph_sentence} bei {ci_phrase}{co_method_phrase}. "
        "{cv_stauung_phrase} {pv_stauung_phrase} "
...
_add_k_package(
    ...
    beurteilung=(
        "{borderline_ph_sentence} bei {pvr_phrase} und {ci_phrase}{co_method_phrase}. "
        "{cv_stauung_phrase} {pv_stauung_phrase} {step_up_sentence} "
...
_add_k_package(
    ...
    beurteilung=(
        ...
        "({mpap_phrase}, {pawp_phrase}, {pvr_phrase}) bei {ci_phrase}{co_method_phrase}. "
        "{cv_stauung_phrase} {pv_stauung_phrase} {step_up_sentence} "
        "{systemic_sentence} {oxygen_sentence} "
        "Aktuell {exam_type_desc}."
    ),
...
_add_k_package(
    ...
    beurteilung=(
        "Schwergradige pulmonale Druck- und Widerstandserhöhung bei {ci_phrase}{co_method_phrase}. "
        "{cv_stauung_phrase} {pv_stauung_phrase} "
        "{systemic_sentence} {oxygen_sentence} "
        "{comparison_sentence} "
...
_add_k_package(
    ...
    beurteilung=(
        "{comparison_sentence} "
        "{cv_stauung_phrase} {pv_stauung_phrase} {systemic_sentence} {oxygen_sentence}"
    ),
    empfehlung=(
...
BUNDLES: Dict[str, Dict[str, List[str]]] = {
    ...
    "K21": {"B": ["K21_B"], "E": ["K21_E"], "P_suggestions": ["P01"]},
}
...
QUICKFINDER: Dict[str, List[str]] = {
    ...
    "PVOD-Verdacht": ["K19"],
    "mPAP-Erhöhung bei normaler PVR": ["K20"],
    "Sekundäre Ursachen/Trigger": ["K21"],
}
