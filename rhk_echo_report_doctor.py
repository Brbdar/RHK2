...
def _ph_screening_summary(ui: Dict[str, Any]) -> Tuple[str, List[str]]:
    ...
    screen = compute_echo_ph_probability(ui)
    if screen.probability is None and pasp is None:
        return "PH Echo-Screening nicht beurteilbar", ["TRV und zusätzliche ESC/ERS-Screeningzeichen nicht ausreichend dokumentiert."]

    if screen.probability is None:
        level = "eingeschränkte Beurteilbarkeit"
    else:
        level_map = {
            "niedrig": "niedrige Wahrscheinlichkeit",
            "intermediär": "intermediäre Wahrscheinlichkeit",
            "hoch": "hohe Wahrscheinlichkeit",
        }
        level = level_map.get(screen.probability, "eingeschränkte Beurteilbarkeit")
    ...
    categories = []
    for label, reasons in (screen.category_reasons or {}).items():
        if reasons:
            categories.append(f"{label}: {', '.join(reasons)}")
    if categories:
        bullets.append(
            f"Zusätzliche ESC/ERS-Zeichen in {screen.category_count} Kategorie(n): " + "; ".join(categories)
        )
    elif not screen.trv_present:
        bullets.append("Keine zusätzlichen ESC/ERS-Zeichen aus >=2 Kategorien dokumentiert.")

    if not screen.trv_present:
        if pasp is not None:
            bullets.append("TRV nicht dokumentiert; PASP allein ersetzt die ESC/ERS-Wahrscheinlichkeitsklassifikation nicht.")
        else:
            bullets.append("TRV nicht dokumentiert; Einordnung basiert nur auf zusätzlichen ESC/ERS-Screeningzeichen.")

    if screen.probability is None:
        return "PH Echo-Screening eingeschränkt beurteilbar", bullets
